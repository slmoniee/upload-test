//[2023-12-13 16:10 Code[FIN] - NewGamePanelJava.java]
//Idea) 기존 JFrame을 JPanel로 변경, 타 클래스에서 본 패널을 생성, 추가, 삭제, 재생성.. 하는 방식으로 구현
//Work With [GameFrame_Decoupling2]
...
public class NewGamePanelJava extends JPanel {
   int g_W = 1024;
   int g_H = 768;
   ...
   WordList_Decoupling WordListJavaInstance = new WordList_Decoupling(); // `WordList` 클래스의 인스턴스를 생성합니다.
   ArrayList<String> wordListJavaShuffled = WordListJavaInstance.getWordListJava();
...
//      출처: https://tikcode.tistory.com/18 [TikLog:티스토리]
   ...
   ////----- ▲ 테스트 코드 영역
      ...
      int boxWidth = 280;
      int boxHeight = 50;          // 이게 이제 박스 크기 가로, 세로 크기 1024*720 하면 사이즈가 적정하고, 스택팅 되는 현상 보면 들어맞음.
...
////        ----- ▲ 테스트 코드 영역
   ...
   class quizLabelMakerJava {
      private int quizLabel_W, quizLabel_H, initial_y; // ????? : private 써야해? -> 외부 간섭 치면 안되는 애들 있어서 private 써야합니다.
...
//      new NewGamePanelJava(); // 객체 생성 (생성자 정의만 하면 안됩니다. 생성을 해야죠.)
//
//   }
